...
class Enemy(newSprite):
     def  __init__(self, filename, framesX =1, framesY=1):
         newSprite.__init__(self, filename, framesX =1, framesY=1)
         self.health = 1
         self.speed = 3
         self.rect.x  = 200
         self.rect.y = 200
     def move(self, frame):
        if self.orientation == 0:
            self.changeImage(frame*4)
            self.rect.x += self.speed
        elif self.orientation == 1:
            self.changeImage(frame*4 + 1)
            self.rect.y -= self.speed
        elif self.orientation == 2:
            self.changeImage(frame*4 + 2)
            self.rect.x += 3
        elif self.orientation == 3:
            self.changeImage(frame*4 +3)
            self.rect.x +=self.speed
            
class Octorok(Enemy):
    def __init__(self):
        Enemy.__init__("Octorok.png", 4, 2)
        self.step = 0
    def move(self, frame):
        self.step +=1
        
        if selfstep == random.randint(20,20):
            self.speed = 0
        if self.step == random.randint(30,40):
